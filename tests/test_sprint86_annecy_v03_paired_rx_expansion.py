...
V04_REVIEW = ROOT / "research/annecy-alpes-leman-v0.4/compatibility-and-source-review.json"
...
assert not PUBLIC_V03_ROUTE.exists(), "Published Annecy v0.3 uses frozen static CSV, not a dynamic route"
...
# Sprint 86 was an Annecy v0.3 internal-candidate milestone. Later sprints may
# legitimately move active_work to another pack, so preserve its historical
...
# If later research exists, it must build on—not rewrite—the Sprint 86/v0.3 result.
if "annecy_v0_4_research" in state:
    v04 = state["annecy_v0_4_research"]
    assert v04["based_on_public_version"] == "0.3"
    assert v04["public_version_remains"] == "0.3"
    assert v04["candidate_memory_count"] == 77
    assert v04["candidate_without_aviation_memory_count"] == 60
    assert v04["candidate_memory_delta"] == 1
    assert v04["added_frequency_mhz"] == 50.5375
    assert v04["published"] is False
    assert V04_REVIEW.is_file()
...
assert not expected_added.intersection(base_freqs)

# Paired-RX cases already fully represented in v0.2 stay zero-delta.
...
# Historical Sprint 86 truth: F1ZTH 50 MHz was deferred at that milestone.
# A later, separately evidenced v0.4 candidate may clear that gate without
# rewriting the historical expansion record.
...
for include_aviation, expected_base_count, expected_v03_count in [(True, 65, 76), (False, 48, 59)]:
    base_temp, old_rows, _old_manifest = build(BASE_BUILDER, include_aviation)
