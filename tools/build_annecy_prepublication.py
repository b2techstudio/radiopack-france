#!/usr/bin/env python3
"""Build a non-public Annecy–Alpes–Léman v0.2 prepublication candidate."""
...
AVIATION_BLOCKS = {
    "Aviation France et bassin genevois",
    "Aviation Suisse",
}
...
def build_prepublication(
    root: Path,
    include_aviation: bool = True,
    notam_check: str = "disabled",
    notam_confirmed_at: str | None = None,
) -> dict[str, Any]:
    readiness_result = readiness.evaluate(root)
    if not readiness_result["ready_for_public_prepublication"]:
        blockers = ", ".join(
            blocker["id"] for blocker in readiness_result["blockers"]
        )
        raise RuntimeError(
            "Prépublication interdite tant que les portes bloquantes ne sont pas validées"
    ...
    if notam_check not in allowed_notam_states:
        raise ValueError(
            f"État NOTAM non autorisé: {notam_check}; attendus: {', '.join(sorted(allowed_notam_states))}"
        )

    internal_candidate = internal.build_candidate(root)
    memories = []
    for item in internal_candidate["memories"]:
        if (
            not include_aviation
            and item["channel"].get("candidate_block") in AVIATION_BLOCKS
        ):
            continue
    ...
    if len(memories) != expected_count:
        raise ValueError(
            f"Total de prépublication inattendu: {len(memories)} au lieu de {expected_count}"
        )
    ...
    if len(locations) != len(set(locations)):
        raise ValueError("Conflit de numéros de mémoire dans la prépublication")
    if len(names) != len(set(names)):
        raise ValueError("Noms de mémoire dupliqués dans la prépublication")
    if len(frequencies) != len(set(frequencies)):
        raise ValueError("Fréquences dupliquées dans la prépublication")
    ...
    return {
        ...
        "rules": [
            "Ce candidat de prépublication reste hors de website/public.",
            "Toutes les mémoires exportées utilisent Duplex=off.",
            ...
            "Une revue finale explicite reste obligatoire avant création du téléchargement public.",
        ],
        "memories": memories,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--notam-check",
        choices=["disabled", "requested_unconfirmed", "user_confirmed"],
        default="disabled",
        help="État du contrôle NOTAM facultatif; n'altère jamais les fréquences du CSV.",
    )
    parser.add_argument(
        "--notam-confirmed-at",
        default=None,
    ...
    candidate = build_prepublication(
        root,
        include_aviation=args.aviation,
        notam_check=args.notam_check,
        notam_confirmed_at=args.notam_confirmed_at,
    )
    json_path, csv_path = write_prepublication(candidate, output_dir)

    print(
        "PREPUBLICATION READY: "
        f"{candidate['memory_count']} memories; "
        f"aviation={'on' if candidate['include_aviation'] else 'off'}; "
        f"notam={candidate['notam']['state']}"
    )
    print(json_path)
    print(csv_path)
    print("NOT PUBLIC: final website export still requires explicit review")
