...
def build(root: Path, as_of: date | None = None) -> dict[str, Any]:
    ...
    items = [
        ...
        {
            "id": "SOURCE_FRESHNESS",
            "label": "Les revalidations externes ne sont pas périmées selon la politique interne.",
            "completed": bool(freshness["release_review_freshness_gate_passed"]),
            "blocking_if_false": True,
        },
        {
            "id": "R3_MORTAIN_RX",
            "label": "La preuve de réception R3 depuis Mortain satisfait le protocole.",
            "completed": bool(station["F1ZBX_R3"]["field_gate_supported"]),
            "blocking_if_false": True,
        },
        {
            "id": "F5ZHA_SOURCE_AND_COVERAGE",
            "label": "F5ZHA a à la fois une couverture utile documentée et une réconciliation autoritative du conflit.",
            "completed": bool(station["F5ZHA_LAVAL"]["promotion_prerequisites_satisfied"]),
            "blocking_if_false": True,
        },
        {
            "id": "F1ZOV_OPERATIONAL_STATUS",
            "label": "L'exploitant local ne marque plus F1ZOV en maintenance.",
            "completed": bool(station["F1ZOV_EQUEURDREVILLE"]["maintenance_cleared"]),
            "blocking_if_false": True,
        },
        {
            "id": "F6ZES_RESOLVED",
            "label": "F6ZES dispose d'une fréquence et d'un mode vérifiés sans valeur devinée.",
            "completed": bool(station["F6ZES_SOURDEVAL"]["frequency_resolved"]),
            "blocking_if_false": True,
        },
        {
            "id": "FINAL_MEMORY_PLAN",
            "label": "La taille finale et les positions publiques sont explicitement définies.",
            "completed": final_memory_plan_defined,
            "blocking_if_false": True,
        },
        {
            "id": "FINAL_REVIEW",
            "label": "La revue finale explicite a été terminée.",
            "completed": publication["review_completed"] is True,
            "blocking_if_false": True,
        },
        {
            "id": "PUBLIC_REGISTRY",
            "label": "Le registre public contient v0.4 seulement après validation séparée.",
            "completed": public_registry_has_v04,
            "blocking_if_false": True,
        },
    ]
    ...
    return {
        ...
        "rules": {
            ...
            "public_registry_is_a_separate_reviewed_change": True,
...
def markdown(data: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Revue complète : **{'oui' if data['release_review_complete'] else 'non'}**",
        "",
    ]
    for item in data["items"]:
        mark = "x" if item["completed"] else " "
        lines.append(f"- [{mark}] **{item['id']}** — {item['label']}")
    lines.extend(["", "Cette checklist est informative et ne publie rien.", ""])
    return "\n".join(lines)
...
def main() -> None:
    ...
    print(f"NORMANDIE V0.4 REVIEW CHECKLIST: {data['completed_count']}/{data['item_count']} complete; release=false")
    print(jp)
    print(mp)
