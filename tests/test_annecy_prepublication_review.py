...
for path in [BUILDER, REVIEW_MAP, PLAN, PUBLIC_ROUTE]:
...
assert review["schema"] == ["location", "name", "frequency_mhz", "mode", "step_khz", "block", "comment_sha256"]
...
def run_builder(*extra_args: str):
    ...
    completed = subprocess.run(
        [sys.executable, str(BUILDER), "--root", str(ROOT), "--output-dir", str(output_dir), *extra_args],
        text=True,
        capture_output=True,
    )
...
def assert_reviewed_csv(actual_rows, expected_rows):
    ...
    for actual, expected in zip(actual_rows, expected_rows):
        ...
        assert actual["URCALL"] == actual["RPT1CALL"] == actual["RPT2CALL"] == actual["DVCODE"] == ""
...
confirmed_temp, _, confirmed_manifest, confirmed_rows, confirmed_bytes = run_builder(
    "--notam-check", "user_confirmed", "--notam-confirmed-at", "2026-08-08T13:15:00+02:00"
)
try:
...
print("Tests Annecy–Alpes–Léman reviewed CSV retained after publication: 65/65 OK")
