...
import { buildMetropolitanPack } from "../../../lib/metropolitanPack";
import { buildChirpCsv, type Channel, type PlacedChannel } from "../../../lib/chirpPack";
...
const buildBfcV03 = (): PlacedChannel[] => {
  const base = buildMetropolitanPack("bourgogne-franche-comte", "v0.2");
  const additions = extraChannels.map((channel, index) => ({
    location: 80 + index,
    block: channel.mode === "AM" ? "AVIATION_V03" : "RADIOAMATEUR_V03",
    channel,
  }));
  return [...base, ...additions].sort((a, b) => a.location - b.location);
};

export const GET: APIRoute = () => {
  const csv = buildChirpCsv(buildBfcV03());
  return new Response(csv, {
    status: 200,
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": 'attachment; filename="radiopack-france-bourgogne-franche-comte-v0.3.csv"',
