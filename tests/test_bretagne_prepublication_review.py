...
assert gates['status'] in {'prepublication_ready_135_explicit_publication_pending','published_immutable_135'} and gates['public_release_allowed'] is False
...
assert (ROOT/'research/bretagne-v0.1/publication-record.json').is_file()
assert 'id: "bretagne"' in (ROOT/'website/src/lib/packRegistry.ts').read_text(encoding='utf-8').lower()
print('Bretagne v0.1 prepublication review: frozen 135 RX memories, review 8/8, zero blockers, aviation/ADRASEC/site metadata deferred, historical review preserved after immutable publication OK')
