...
if state["current_sprint"] > 73:
    assert state["active_work"]["pack"] == "Bretagne"
    assert state["active_work"]["target_version"] == "0.2"
    assert state["active_work"]["published_base_version"] == "0.1"
    assert state["active_work"]["published_base_memory_count"] == 135
print("Sprint 73: Bretagne v0.1 published immutable at 135 RX memories; deferred v0.2 scope preserved and remains auditable after later-version initialization OK")
